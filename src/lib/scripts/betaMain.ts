import { map } from "./betaMap";
import { utils } from "./betaUtil";
import { drawMap } from "./beta_draw_map";

// CANVAS
const canvasObj = utils.createCanvas({
    width: 320,
    height: 240
}),
...
// CREATE GRID
const grid = map.createGridObject(12, 8);
grid.xOffset = 0;
grid.yOffset = 0;
...
const loop = function () {
    ...
    const offsets = map.clampedOffsets(grid, canvas, ratio);
...
canvas.addEventListener('mousedown', function (e) {
    ...
    e.preventDefault();
    mousedown = true;
    const cell = map.getCellFromCanvasPoint(grid, pos.x / ratio, pos.y / ratio);
...
canvas.addEventListener('mousemove', function (e) {
    const canvas = e.target,
        ...
        deltas = map.getPointerMovementDeltas(grid, canvas, x, y);
    if (mousedown) {
